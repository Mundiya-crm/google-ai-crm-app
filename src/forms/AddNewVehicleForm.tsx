...
import { vehicleMakes, vehicleModelsByMake, vehicleColors, transmissionTypes, fuelTypes, vehicleLocations } from '../constants/vehicleOptions';
...
const initialFormState: Omit<Vehicle, 'id' | 'stockNumber'> = {
    purchaseType: 'import',
    supplierId: '',
    salespersonId: '',
    salespersonSubPrefix: '',
    lotNumber: '',
    ...
    fuelType: 'Petrol',
    totalCnf: defaultCurrencyValue,
    purchasePrice: defaultKESCurrencyValue,
};
...
export const AddNewVehicleForm: React.FC<AddNewVehicleFormProps> = ({ setView, purchaseType }) => {
    ...
    useEffect(() => {
        if (purchaseType) {
            setFormData(prev => ({ ...prev, purchaseType, supplierId: '', salespersonId: '' }));
        }
    }, [purchaseType]);
    ...
    useEffect(() => {
        // Reset supplier if it doesn't match the current purchase type
        const currentSupplier = suppliers.find(s => s.id === formData.supplierId);
        if (currentSupplier && currentSupplier.type !== formData.purchaseType) {
            setFormData(prev => ({ ...prev, supplierId: '', salespersonId: '' }));
        }
    }, [formData.purchaseType, formData.supplierId, suppliers]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!formData.chassisNumber || !formData.make || !formData.model || !formData.supplierId || !formData.salespersonId) {
            setMessage({ type: 'error', text: 'Chassis #, Make, Model, Supplier, and Salesperson are required.'});
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            const supplierStockCount = vehicles.filter(v => v.supplierId === formData.supplierId).length + 1;
            const stockNumber = `${stockPrefix}-${String(supplierStockCount).padStart(3, '0')}`;
            
            const newVehicle: Vehicle = { ...formData, id: newId, stockNumber };
            setVehicles(prev => [...prev, newVehicle]);
            logActivity('VEHICLE_CREATED', `created ${newVehicle.purchaseType} vehicle ${stockNumber} (${newVehicle.make} ${newVehicle.model})`, 'vehicle', newId);


            setMessage({ type: 'success', text: `Vehicle ${stockNumber} added successfully!` });
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-content-secondary p-4 rounded-lg border border-content-primary shadow-lg h-full flex flex-col">
            <div className="flex justify-between items-center mb-3">
                <h2 className="text-xl font-bold text-content-primary">Add New Vehicle</h2>
            ...
            <div className='flex-grow overflow-y-auto pt-2 pb-6 pr-2 space-y-4'>
                <div className="p-2 bg-content-tertiary/20 rounded-md">
                    <div className="flex rounded-md p-1 bg-tertiary/50">
                        {(['import', 'local'] as const).map(type => (
                            <button key={type} type="button" onClick={() => setFormData({...formData, purchaseType: type})} className={`flex-1 text-sm font-bold py-1.5 rounded-md transition-colors capitalize ${formData.purchaseType === type ? 'bg-accent text-inverted' : 'hover:bg-tertiary'}`}>
                                {type} Purchase
                            </button>
                        ))}
                    </div>
                ...
                <SubSection title="Core Details">
                    ...
                    <FormRow><Label htmlFor="model">Model</Label>
                         <Select name="model" value={formData.model} onChange={handleInputChange} disabled={!formData.make} required>
                ...
                <SubSection title="Purchase & Costing">
                    ...
                    <FormRow><Label htmlFor="salespersonId">Salesperson</Label>
                     ...
                     {formData.purchaseType === 'import' ? (
                        <FormRow><Label htmlFor="totalCnf">Total C&F</Label><CurrencyInput id="totalCnf" value={formData.totalCnf || defaultCurrencyValue} onChange={val => handleCurrencyChange('totalCnf', val)} /></FormRow>
                     ) : (
                        <FormRow><Label htmlFor="purchasePrice">Purchase Price</Label><CurrencyInput id="purchasePrice" value={formData.purchasePrice || defaultKESCurrencyValue} onChange={val => handleCurrencyChange('purchasePrice', val)} forceCurrency="KES" /></FormRow>
                     )}
                </SubSection>
