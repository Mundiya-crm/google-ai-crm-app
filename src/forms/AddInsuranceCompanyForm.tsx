...
const INSURANCE_PAYABLES_ACCOUNT_ID = 16;
...
export const AddInsuranceCompanyForm: React.FC<{ setView: (view: string) => void }> = ({ setView }) => {
    const { insuranceCompanies, setInsuranceCompanies, chartOfAccounts, setChartOfAccounts } = useData();
    const [formData, setFormData] = useState(initialFormData);
    const [isLoading, setIsLoading] = useState(false);
    const [message, setMessage] = useState<{ type: 'success' | 'error', text: string } | null>(null);
    const [duplicateCompany, setDuplicateCompany] = useState<InsuranceCompany | null>(null);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const existingCompany = insuranceCompanies.find(c => c.name.toLowerCase() === trimmedName);
        if (existingCompany) {
            setMessage({ type: 'error', text: `An insurance company named "${formData.name.trim()}" already exists.` });
            setDuplicateCompany(existingCompany);
            setIsLoading(false);
            return;
        }
        
        const parentAccount = chartOfAccounts.find(a => a.id === INSURANCE_PAYABLES_ACCOUNT_ID);
        const baseCode = parentAccount ? parentAccount.code : '2014';
        const existingSubAccounts = chartOfAccounts.filter(a => a.parentId === INSURANCE_PAYABLES_ACCOUNT_ID).length;
        ...
        const newCompany: InsuranceCompany = {
            ...
            salespersons: [
                {
                    id: `${formData.name.toLowerCase().replace(/\s+/g, '')}_${(formData.contactPerson || 'default').toLowerCase().replace(/\s+/g, '')}`,
        ...
        const newAccount = {
            ...
            parentId: INSURANCE_PAYABLES_ACCOUNT_ID,
        };
        setChartOfAccounts(prev => [...prev, newAccount].sort((a,b) => a.code.localeCompare(b.code)));
        
        setMessage({ type: 'success', text: `Insurance Company '${formData.name}' and its A/P account have been created successfully!` });
        setFormData(initialFormData);
        setIsLoading(false);

        setTimeout(() => setView('insurance'), 2000);
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-content-secondary p-6 rounded-md border border-content-primary shadow-inner h-full overflow-y-auto">
            <div className="flex justify-between items-center mb-6 border-b border-content-primary pb-4">
                 <h2 className="text-2xl font-bold text-content-primary">Add New Insurance Company</h2>
                 <button type="button" onClick={() => setView('insurance')} className="text-sm font-semibold text-accent hover:underline">
            ...
            <div className="space-y-4 max-w-2xl mx-auto">
                <p className="text-sm text-content-secondary mb-4 bg-content-tertiary/50 p-3 rounded-md border border-content-primary">
                    Enter the company's information below. An associated 'Accounts Payable' sub-account will be automatically created.
                </p>
                <FormRow>
