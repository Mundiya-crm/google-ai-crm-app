...
import type { Currency, CurrencyValue } from '../constants/vehicleData';
...
export const CurrencyInput: React.FC<CurrencyInputProps> = ({ id, value, onChange, disabled, forceCurrency }) => {
    ...
    useEffect(() => {
        if (forceCurrency && value.currency !== forceCurrency) {
            handleCurrencyChange(forceCurrency);
        }
    }, [forceCurrency, value.currency]);
    ...
    const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const currentRate = forceCurrency === 'KES' ? 1 : (value.rate || 1);
        const kesAmount = value.currency === 'KES' ? numericValue : numericValue * currentRate;
    ...
    const isCurrencyForced = !!forceCurrency;
    const showRateInput = !isCurrencyForced && value.currency !== 'KES';
        ...
        <div className="md:col-span-2 w-full grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div className="relative sm:col-span-2">
                ...
                <select 
                    ...
                    className="absolute left-0 top-0 h-full bg-content-tertiary border-r border-content-secondary text-accent font-bold text-sm px-2 rounded-l-btn focus:outline-none disabled:bg-content-tertiary/50 disabled:text-content-secondary"
                    disabled={disabled || isCurrencyForced}
                    aria-label="Currency"
                >
            ...
            <div className={`text-right sm:col-span-1 flex items-center justify-end ${!showRateInput ? 'sm:col-span-2' : ''}`}>
