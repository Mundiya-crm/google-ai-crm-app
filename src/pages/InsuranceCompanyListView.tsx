...
interface InsuranceCompanyListViewProps {
    setView: (view: string, queryParams?: Record<string, any>) => void;
}

export const InsuranceCompanyListView: React.FC<InsuranceCompanyListViewProps> = ({ setView }) => {
    const { insuranceCompanies: companies } = useData();
    ...
    return (
        <div className="bg-content-secondary p-6 rounded-md border border-content-primary shadow-inner h-full flex flex-col">
            <div className="flex flex-wrap justify-between items-center gap-4 mb-4 border-b border-content-primary pb-3">
                <h2 className="text-2xl font-bold text-content-primary">Insurance Companies</h2>
                <button 
                    onClick={() => setView('add_insurance_company')}
                            ...
                            <tr key={company.id} className="text-xs border-b border-content-primary hover:bg-content-tertiary/50">
                                ...
                                <td className="py-2 px-3">
                                    <div className="flex items-center justify-center gap-3">
                                        ...
                                        <button onClick={() => setView('insurance_ledger', { accountCode: company.apAccountCode, companyName: company.name })} className="text-accent hover:underline font-semibold text-xs">Ledger</button>
